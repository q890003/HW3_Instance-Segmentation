...
def train_model(
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ["train", "val"]:
            ...
            loss_classifier, loss_box_reg, loss_mask, loss_rpn_box_reg = 0., 0., 0., 0.
            # Iterate over data.
            ...
            for i, (imgs, _targets) in enumerate(dataloaders[phase]):
                ...
                if phase == 'train':
                    ...
                    if (current_num_data % (32) == 0):
                        print("epoch:{epoch}/{total_epoch} [{learned_data}/{data}], "\
                              "time: {minute:.0f}m {sec:.0f}s, Batch Loss: {loss:.4f}".format(
                                epoch=epoch, total_epoch=(num_epochs-1), 
                                learned_data=current_num_data, data=all_data,
                                minute=minute_batch, sec=second_batch, loss=loss.item(),
                            )
                        )
            ...
            if phase == "train":
                scheduler.step()
                print("Train loss:{loss:.4f}".format(
                ...
                print("Val Loss: {loss:.4f}".format(
                        loss=val_loss_avg,))
                
                print("\nepoch time:{minute:.0f}m {sec:.0f}s".format(
                        minute=(time_epoch/60), sec=(time_epoch%60),))
                # save checkpoints
                if val_loss_avg < best_loss:
                    best_loss = val_loss_avg
                    best_model_wts = copy.deepcopy(model.state_dict())
                    torch.save(best_model_wts, config.ckpt_dir + config.model_name +  \
                               'alldataaug__SGD_LrStep_resnest50_val_loss_no_setGrad' \
                               '_epoch{epoch}_loss{loss}'.format(epoch=epoch, loss=val_loss_avg))
...
if __name__ == "__main__":
    ...
    if checkpoint is None:
        ...
        exp_lr_scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer_ft, milestones, gamma=0.1, last_epoch=-1)
